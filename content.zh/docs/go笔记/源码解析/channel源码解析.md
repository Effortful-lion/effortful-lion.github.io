...
```text
1. 关闭的 channel：
读：无阻塞（先读有效数据，后永久读零值，ok 区分）；
写：立即 panic（可被 select 选中）。

2. nil 的 channel：
读写：永久阻塞（无唤醒可能，全 goroutine 阻塞触发死锁）。

3. 正常开启的 channel： （有缓冲能用就用，不能用能直接通信就通信，不能就阻塞）
缓冲 channel：
✅ 读：优先读缓冲区 → 空则看发送者（有则直接读，无则阻塞）；
✅ 写：优先写缓冲区 → 满则看接收者（有则直接写，无则阻塞，被关唤醒 panic）；
无缓冲 channel：
✅ 读写：仅看对方等待 goroutine（有则无阻塞 1 次拷贝，无则阻塞）。

4. 多次关闭 channel 则 panic
```
